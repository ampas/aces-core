...
This node maps the input domain to the output range by scaling and offsetting values. The \xml{Range} node can also be used to clamp values on only the top or bottom end.
...
The \xml{Range} element can also be used in to clamp values. In such instances, no offset is applied, and the formula simplifies because only one pair of min or max values are required. 
