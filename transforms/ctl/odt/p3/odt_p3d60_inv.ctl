...
)
{
  ...
  // Undo CCTF
  float tmp[3];
  tmp[0] = pow((outputCV[0] - CV_BLACK) / (CV_WHITE - CV_BLACK), DISPGAMMA);
  tmp[1] = pow((outputCV[1] - CV_BLACK) / (CV_WHITE - CV_BLACK), DISPGAMMA);
  tmp[2] = pow((outputCV[2] - CV_BLACK) / (CV_WHITE - CV_BLACK), DISPGAMMA);
